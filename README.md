...
机器绑定只使用 `HOST_BIND` 与 `PORT`。容器默认监听本机地址；需要跨机器读取时，应通过 Tailscale、WireGuard 或 HTTPS 反向代理提供内部地址。`INTERNAL_API_TOKEN` 只保留在两个后端，不能进入网页、浏览器存储或网络响应。

## 启动

```bash
cp .env.example .env
# 在 .env 中填写一个足够长的 INTERNAL_API_TOKEN
